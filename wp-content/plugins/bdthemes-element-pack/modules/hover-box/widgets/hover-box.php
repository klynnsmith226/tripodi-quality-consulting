...
class Hover_Box extends Module_Base {
	...
	protected function register_controls() {
		...
		$repeater->add_control(
		    ...
		    [
			    ...
			    'raw' => __( 'Note: This option will work if the background image is empty.', 'bdthemes-element-pack' ),
		...
		$this->add_control(
			...
			[
				...
				'description' => __( 'Set default item by inserting the item\'s numeric position (i.e. 1 or 2 or 3 or ...) The numeric position reads from the top-left corner as 1st and continues to the right side.', 'bdthemes-element-pack' ),
				'type'        => Controls_Manager::TEXT,
				'default'	  => '1',
				'condition' => [
					'_skin!' => 'bdt-flexure',
		...
		$this->add_control(
			...
			[
				'label' => __( 'Advanced Radius', 'bdthemes-element-pack' ) . BDTEP_NC,
				'type'  => Controls_Manager::SWITCHER,
			]
		);
	...
	protected function render() {
		...
		if ( 'yes' == $settings['box_image_effect'] and 'effect-1' == $settings['box_image_effect_select'] ) {
			$this->add_render_attribute( 'hover_box', 'class', 'bdt-ep-hover-box-img-effect bdt-effect-1' );
		} elseif ( 'yes' == $settings['box_image_effect'] and 'effect-2' == $settings['box_image_effect_select'] ) {
			$this->add_render_attribute( 'hover_box', 'class', 'bdt-ep-hover-box-img-effect bdt-effect-2' );
		}
						...
						<div>
							<div <?php echo ( $this->get_render_attribute_string( 'box-item' ) ); ?> data-id="<?php echo esc_attr($tab_id); ?>">
								...
								<?php if ( $item['hover_box_content'] && ( 'yes' == $settings['show_content'] ) ) : ?>
									<div class="bdt-ep-hover-box-text <?php echo $text_hide_on_setup; ?>">
