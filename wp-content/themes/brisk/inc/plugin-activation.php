...
function uicore_ajax_plugins(){
    if ( ! check_ajax_referer( 'uicore_setup_nonce', 'wpnonce' ) || empty( $_POST['slug'] ) ) {
        wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'No Slug Found', 'brisk' ) ) );
    }
    $json = array();
    ...
    foreach ( $plugins as $plugin ) {
        if ( $_POST['slug'] == $plugin['slug'] ) {
            $json = array(
                ...
                'action2'       => - 1,
                'message'       => esc_html__( 'Installing...', 'brisk' ),
            );
            break;
        }
    }
...
if (!function_exists('brisk_plugins_activation')) :

    function brisk_plugins_activation()
    {

        $plugins = uicore_get_plugins();

        $config = array(
            'id'           => 'brisk',            // Unique ID for hashing notices for multiple instances of TGMPA.
            ...
            'strings'      => array(
                'page_title'                      => __( 'Install Recommended Plugins', 'brisk' ),
                'menu_title'                      => __( 'Install Plugins', 'brisk' )
            )
        );
...
add_action('tgmpa_register', 'brisk_plugins_activation');
 ...
 */
function brisk_get_plugin_activation_link( $plugin_base_name, $slug, $plugin_filename ) {
 ...
 */
function brisk_get_plugin_install_link( $plugin_slug ){
 ...
 */
function brisk_make_html_attributes( $attrs = array() ){
...
function brisk_plugin_notice(){
    ...
    $plugins_title     = array(
        __('UiCore Framework', 'brisk'),
        __('Elementor', 'brisk'),
        __('Element Pack', 'brisk')
    );
    ...
    foreach ( $not_installed_or_activated_plugins_id as $key => $value ) {
        ...
        $links_attrs[$key] = array(
            ...
            'data-activating-label' => sprintf( __( 'Activating %s', 'brisk' ), $progress_text_and_title ),
            'data-installing-label' => sprintf( __( 'Installing %s', 'brisk' ), $progress_text_and_title ),
            'data-activate-label'   => sprintf( __( 'Activate %s'  , 'brisk' ), $progress_text_and_title ),
            'data-install-label'    => sprintf( __( 'Install %s'   , 'brisk' ), $progress_text_and_title ),

            'data-activate-url'     => brisk_get_plugin_activation_link( $plugins_base_name[$value], $plugins_slug[$value], $plugins_filename[$value] ),
            'data-install-url'      => brisk_get_plugin_install_link( $plugins_slug[$value] ),
        ...
        if( ! isset( $installed_plugins[ $plugins_base_name[$value] ] ) ){
            ...
            $links_attrs[$key]['button_label'] = sprintf( esc_html__( 'Install %s', 'brisk' ), $progress_text_and_title );
            ...
            $links_attrs[$key]['button_label'] = sprintf( esc_html__( 'Activate %s', 'brisk' ), $progress_text_and_title );
        }
    }
?>
<script>
...
.uicore-close:hover::before{
    ...
    <div class="uicore-msg" style="background-image:url(<?php echo get_template_directory_uri(); ?>/assets/img/demo-import-bg.jpg)">
        <h3 class="uicore-title-default"><?php printf( __( 'Thanks for choosing %s', 'brisk' ), UICORE_THEME_NAME ); ?></h3>
        <p class="uicore-description-default"><?php echo __( 'Please install and activate the required plugins to gain access to all the theme functionalities and features.', 'brisk' ); ?></p>
        <p class="submit">
            <a class="uicore-btn-default" data-info='<?php echo wp_json_encode( $links_attrs);?>' >Install Required Plugins</a>
            <a href="<?php echo esc_url( wp_nonce_url( add_query_arg( 'uicore-fw-notice', 'install' ), 'uicore-fw-notice', '_notice_nonce' ) ); ?>" class="notice-dismiss uicore-close"><span class="screen-reader-text"><?php _e( 'Skip', 'brisk' ); ?></span></a>
        </p>
    </div>
<?php
}
add_action( 'admin_notices', 'brisk_plugin_notice' );
...
function uicore_hide_notice() {
    ...
    if ( isset( $_GET['uicore-fw-notice'] ) && isset( $_GET['_notice_nonce'] ) ) {
        if ( ! wp_verify_nonce( $_GET['_notice_nonce'], 'uicore-fw-notice' ) ) {
            wp_die( __( 'Authorization failed. Please refresh the page and try again.', 'brisk' ) );
        }
